...
export default function Page6() {
  ...
  useEffect(() => {
    const fetchFashionProducts = async () => {
      try {
        const res = await fetch("https://surpriselly.onrender.com/api/products?category=Fashion%20%26%20Accessories");
        ...
        const normalized = data.map((item) => ({
          ...
          category: item.category || "Fashion & Accessories",
          shop: item.shop || "Clothing",
        }));
        ...
        setLoading(false);
        addProducts(uniqueItems);
      } catch (err) {
        console.error("Failed to fetch Fashion & Accessories products:", err);
        setLoading(false);
      }
    };
